...
export function calculateAge(
  ...
  const ageByYear = Operation.sub(
    Operation.sub(currentYear, dobYear),
  ...
  const dayGt = Operation.lessThan(
    dobDay,
    Operation.add(currentDay, Operation.constant(Field(1)))
  )
  const isHigherDayOnSameMonth = Operation.and(monthEq, dayGt)
  // Final age calculation
  const age = Operation.add(
    ageByYear,
    Operation.ifThenElse(
      Operation.or(monthGt, isHigherDayOnSameMonth),
      Operation.constant(Field(1)),
...
export async function ageMoreThan18Spec() {
  return Spec(
    ...
    ({ credential, currentDay, currentMonth, currentYear }) => {
      ...
      const ageMoreThan18 = Operation.lessThanEq(
