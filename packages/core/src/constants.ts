const MAX_FIELD_BYTE_SIZE = 31
const PHOTO_PACK_SIZE = 32
const DELIMITER_ARRAY_SIZE = 18
const DATA_ARRAY_SIZE = 1536

export const DELIMITER_POSITION = {
  REFERENCE_ID: 2,
  NAME: 3,
  DOB: 4,
  GENDER: 5,
  PINCODE: 11,
  STATE: 13,
  PHOTO: 18,
}

export {
  MAX_FIELD_BYTE_SIZE,
  PHOTO_PACK_SIZE,
  DELIMITER_ARRAY_SIZE,
  DATA_ARRAY_SIZE,
}

export const DEMO_PRIVATEKEY =
  'EKFZg3P2XGXXsDVSuZX55tww9mpB8a7nwEZ9KYdzpwK3hJNDTshR'

// Hard-coded public key and QR data that can be obtained from certificates (https://uidai.gov.in/en/916-developer-section/data-and-downloads-section/19388-uidai-certificate-details-2.html)
export const AADHAAR_TEST_PUBLIC_KEY =
  ''
export const AADHAAR_TEST_PUBLIC_KEY_HASH =
  '189363241002317594217666892427360168339122033216079952468438671779607280635'

// ./src/assets/uidai_offline_publickey_26022021.cer
export const AADHAAR_PROD_PUBLIC_KEY =
  ''
export const AADHAAR_PROD_PUBLIC_KEY_HASH =
  '10524837369849002496178907184531379210771062910353927729724796636708237406835'

export const MINA_NODE_ENDPOINT =
  'https://api.minascan.io/node/devnet/v1/graphql'
export const MINA_ARCHIVE_ENDPOINT =
  'https://api.minascan.io/archive/devnet/v1/graphql'
