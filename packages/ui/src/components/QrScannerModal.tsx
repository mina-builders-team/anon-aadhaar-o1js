  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`Scan ${aadhaarMode === 'test' ? 'Test ' : ''}Aadhaar QR Code`}>
      ...
      <div className="flex border-b border-gray-700 mb-4">
        <button
          className={`flex-1 py-2 px-4 text-center ${mode === 'scan' ? 'border-b-2 border-blue-500 text-blue-500' : 'text-gray-400'}`}
          onClick={() => { setMode('scan'); handleReset(); }}
        >
          Scan QR
        </button>
        <button
          className={`flex-1 py-2 px-4 text-center ${mode === 'upload' ? 'border-b-2 border-blue-500 text-blue-500' : 'text-gray-400'}`}
      ...
      {aadhaarDetails && qrNumericString && (
        <div className="mt-4 p-4 border border-gray-700 rounded-lg bg-gray-800">
          <h3 className="text-lg font-semibold mb-2 text-white">Aadhaar Details</h3>
          <div className="space-y-2 text-gray-300">
            <p><span className="font-medium">Name:</span> {aadhaarDetails.name}</p>
          </div>
