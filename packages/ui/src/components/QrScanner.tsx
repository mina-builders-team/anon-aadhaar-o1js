...
export const QrScanner = ({ onScan, isEnabled = true }: QrScannerProps) => {
  ...
  useEffect(() => {
    ...
    const start = async () => {
      ...
      try {
        ...
        const callback = (result: Result | undefined, err: unknown) => {
          if (result) {
    ...
    <div className="relative w-full max-w-md mx-auto min-h-[360px]">
      ...
      <div className="mt-2 text-center">
        <p className="text-sm text-gray-600">
          {scanning ? 'Point camera at an Aadhaar QR code' : 'Scanner inactive'}
        </p>
      </div>
    </div>
  )
