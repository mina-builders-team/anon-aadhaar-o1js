...
import { DEMO_PRIVATEKEY, TEST_DATA, AADHAAR_TEST_PUBLIC_KEY } from "anon-aadhaar-o1js";
...
export default function BenchmarkPage() {
  ...
  const [times, setTimes] = useState<{ init?: number; verifier?: number; extractor?: number; credential?: number; total?: number; totalAll?: number; }>({});
  ...
  const verifierProxyRef = useRef<Comlink.Remote<SignatureWorkerAPI> | null>(null);
  const extractorProxyRef = useRef<Comlink.Remote<ExtractorWorkerAPI> | null>(null);
  const credentialProxyRef = useRef<Comlink.Remote<CredentialWorkerAPI> | null>(null);
  ...
  const ensureWorkers = async () => {
    if (!verifierWorkerRef.current) {
      verifierWorkerRef.current = new Worker(new URL("../../worker/verifierWorker.ts", import.meta.url), { type: "module" });
      verifierProxyRef.current = Comlink.wrap<SignatureWorkerAPI>(verifierWorkerRef.current);
    }
    if (!extractorWorkerRef.current) {
      extractorWorkerRef.current = new Worker(new URL("../../worker/extractorWorker.ts", import.meta.url), { type: "module" });
      extractorProxyRef.current = Comlink.wrap<ExtractorWorkerAPI>(extractorWorkerRef.current);
    }
    if (!credentialWorkerRef.current) {
      credentialWorkerRef.current = new Worker(new URL("../../worker/credentialWorker.ts", import.meta.url), { type: "module" });
      credentialProxyRef.current = Comlink.wrap<CredentialWorkerAPI>(credentialWorkerRef.current);
    }
    return {
  ...
  const handleRun = async () => {
    try {
      ...
      const { verifierProxy, extractorProxy, credentialProxy } = await ensureWorkers();
      ...
      const vProof = await verifierProxy.verifySignature(TEST_DATA, AADHAAR_TEST_PUBLIC_KEY);
      ...
      const eProof = await extractorProxy.extract(vProof, TEST_DATA, AADHAAR_TEST_PUBLIC_KEY);
      ...
      const credentialJson = await credentialProxy.createCredential(eProof, owner.toBase58());
    ...
    <main className="flex min-h-screen flex-col items-center justify-center p-10 bg-gray-900 text-white">
      <div className="w-full max-w-xl p-6 space-y-6 bg-gray-800 rounded-lg shadow-lg">
        ...
        <div className="p-4 rounded-lg bg-gray-800/60 border border-gray-700">
          ...
          {resultOk === true && (
            <p className="text-green-400 text-sm mt-2">Credential created successfully.</p>
          )}
          {resultOk === false && !error && (
            <p className="text-red-300 text-sm mt-2">Credential creation failed.</p>
          )}
        </div>
      </div>
    </main>
