...
import {
  AADHAAR_PROD_PUBLIC_KEY_HASH,
  AADHAAR_TEST_PUBLIC_KEY_HASH,
...
export async function POST(req: Request) {
  try {
    ...
    if (
      environment === 'prod' &&
      outputClaim.pubKeyHash.toString() !== AADHAAR_PROD_PUBLIC_KEY_HASH
    ) {
      return NextResponse.json(
        { ok: false, error: 'verification_failed, pubKeyHash does not match' },
        { status: 200 }
      )
    } else if (
      environment === 'test' &&
      outputClaim.pubKeyHash.toString() !== AADHAAR_TEST_PUBLIC_KEY_HASH
    ) {
      return NextResponse.json(
        { ok: false, error: 'verification_failed, pubKeyHash does not match' },
        { status: 200 }
    ...
    return NextResponse.json({
      ok: true,
      outputClaim: JSON.stringify(outputClaim),
    })
  } catch (e) {
